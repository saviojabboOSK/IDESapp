...
class GraphCard(QWidget):
    ...
    # ------------------------------------------------------------------ UI
    def _build_ui(self):
        self.setMinimumHeight(220)                           # ISSUE #2: fix
        ...
        self.fav_btn.setStyleSheet("QPushButton{background:transparent;}")  # ISSUE #3
        self.fav_btn.clicked.connect(self.toggle_fav)

        del_btn = QPushButton("🗑 Delete")
        del_btn.setStyleSheet("QPushButton{background:transparent;}")       # ISSUE #3
    ...
    def toggle_fav(self):
        ...
        if self.is_fav:  # currently fav -> unfav
            ...
            self.home_widget = HomeGraph(
                self.x, self.y, self,
                x_labels=self.x_labels, is_date=self.is_date,
